...
// ── i18n setup ───────────────────────────────────────────────────────────────────
// Source strings are in English. Translations live in po/<lang>.po and are
// compiled to ~/.local/share/locale/<lang>/LC_MESSAGES/<UUID>.mo by setup.sh.
// Cinnamon auto-translates settings-schema.json via metadata.json gettext-domain.
...
class OutlookCalendarApplet extends Applet.TextIconApplet {
    ...
    constructor(metadata, orientation, panelHeight, instanceId) {
        ...
        this.set_applet_tooltip(_("Outlook - loading..."));
    ...
    _migrateLegacyConfig() {
        try {
            ...
            if (legacy && legacy.ics_url) {
                this.calendars = [{ name: "Outlook", url: legacy.ics_url, color: "#1e88e5", enabled: true }];
                this.settings.setValue("calendars", this.calendars);
                global.log("[Outlook] Legacy config migrated.");
            }
        } catch (e) { global.logError("[Outlook] Migration failed: " + e); }
    }
    ...
    _buildMenu() {
        ...
        // Settings button. The previous implementation used Util.spawnCommandLine
        // from the popup activate signal, which silently failed on some Cinnamon
        // builds. Using Util.spawn with an explicit argv array and an idle
        // dispatch (so the menu can finish closing) works reliably.
    ...
    _openSettings() {
        let argv = ["xlet-settings", "applet", UUID, String(this.instance_id)];
        global.log("[Outlook] Opening settings: " + argv.join(" "));
        this._menu.close(true);
        // Defer to next idle so the menu fully closes before we spawn xlet-settings.
        Mainloop.idle_add(() => {
            try {
                Util.spawn(argv);
            } catch (e) {
                global.logError("[Outlook] Util.spawn failed: " + e + " — trying spawnCommandLine fallback");
                try {
                    Util.spawnCommandLine(argv.join(" "));
                } catch (e2) {
                    global.logError("[Outlook] Both spawn methods failed: " + e2);
                }
            }
            return false;
        });
    ...
    _handleFetchOutput(stdout, stderr) {
        ...
        catch (e) { this._setError(_("Error parsing response")); global.logError("[Outlook] JSON: " + e); return; }
    ...
    _updateDisplay() {
        ...
        // Mode 1: No Display (most restrictive)
        if (!this.showInPanel) {
        ...
        if (!m) {
            this.hide_applet_label(false);
            this.set_applet_label(this.hiddenMode ? "—" : _("No meetings"));
        ...
        // Mode 2: Hidden Mode (privacy - countdown only)
        if (this.hiddenMode) {
            let label, tooltip;
            if (live) {
        ...
        if (live) {
            let mins = Math.round((Date.now() - new Date(live.start).getTime()) / 60000);
            label   = "◎ " + live.subject + "  (" + _f("%d min ago", mins) + ")";
